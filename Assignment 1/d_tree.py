...
if __name__ == '__main__':
    ...
    for i in range(1, 34):
        ...
        # categorise by rounding to nearest 0.5, and multiplying by 10
        # to get category e.g. 8.3 => 85
        # (train on train and validate sets?)
        y = []
        for j in train2_y:
            y.append(categorise(j))
        ...
        # show tree:
        #names = ['avg_mood', 'avg_arousal', 'avg_valence', 'avg_activity',
        #         'avg_screen_time']

        #class_names = [str(x) for x in sorted(set(y))]
        #dot_data = tree.export_graphviz(clf_gini, out_file=None,
        #                                feature_names=names,
        #                                class_names=class_names, 
        #                                filled=True)
        #graph = graphviz.Source(dot_data)
        #graph.render('mood', view=True)

        # evaluate
        predictions = clf_gini.predict(test_x)/10

        # categorise the observed data for fair comparision
        obs = []
        for j in test_y:
            obs.append(categorise(j)/10)
